...
export class SurveyGroupController {
    ...
    req,) {
        ...
        let ret = await this.SurveyGroupService.create({
            name: value.name,
            ownerId: userId
          });
      return {
    ...
    if (error) {
      this.logger.error(
        `GetGroupListDto validate failed: ${error.message}`,
      );
    ...
    const pageSize = Number(value.pageSize);
      ...
      let { total, list, allList } = await this.SurveyGroupService.findAll(userId, value.name, skip, pageSize);
    ...
    req,) {
        ...
        let ret = await this.SurveyGroupService.update(id, value)
      return {
        code: 200,
        ret,
      }
