...
import { getPermissionList, getCollaborator, collaboratorBatchSave } from '@/management/api/space'
import { type IMember, SurveyPermissions, surveyPermissionsLabels } from '@/management/utils/types/workSpace'
import { CODE_MAP } from '@/management/api/base'
// console.log({SurveyPermissions, surveyPermissionsLabels})
...
watch(() => props.modifyId, async (val: string) => {
  if(val) {
    try {
      const res: any = await getCollaborator(val)
      if (res.code === CODE_MAP.SUCCESS) {
  ...
  ruleForm.value?.validate(async (valid: boolean) => {
    if (valid) {
      try {
        ...
        const res: any = await collaboratorBatchSave({
...
const handleMemberSelect = (val: string, label: string) => {
  formModel.value.members.push({ userId: val, username: label, role: [SurveyPermissions.SurveyManage, SurveyPermissions.DataManage, SurveyPermissions.CollaboratorManage] })
}
